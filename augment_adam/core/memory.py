"""Memory management for the Augment Adam assistant.
...
class MemoryManager:
    """Core memory management for the Augment Adam assistant.
    ...
    def __init__(
        ...
        self.persist_dir = persist_dir or os.path.expanduser("~/.augment_adam/memory")
    ...
    def _init_collections(self, collection_name: str) -> None:
        ...
        try:
            ...
            self.collections["main"] = self.client.get_or_create_collection(
                ...
                metadata={"description": "Main memory collection for Augment Adam"}
            )
...
default_memory: Optional[MemoryManager] = None
...
def get_memory(
    ...
    try:
        if default_memory is None:
            ...
            # Use provided values or defaults from settings
            persist_dir = persist_dir or os.path.expanduser("~/.augment_adam/memory")
            collection_name = collection_name or "augment_adam_memory"

            # Create the memory instance
            default_memory = MemoryManager(persist_dir, collection_name)
