...
from augment_adam.context_engine.context_manager import (
    ContextWindow,
    get_context_manager,
)
...
class MemoryConfiguration:
    ...
    def get_allocation_for_role(self, role: str) -> MemoryAllocation:
        ...
        """
        return self.role_configurations.get(
            role.lower(), self.role_configurations["default"]
        )
    ...
    def get_context_window_size(self, model_name: str) -> int:
        ...
        return self.model_context_windows.get(
            model_name, self.model_context_windows["default"]
        )
    ...
    def configure_memory_for_agent(
        ...
        allocation.context_window_size = min(
            allocation.context_window_size, context_window_size
        )
        ...
        context_window = context_manager.create_context_window(
            name=f"{agent_name}_window", max_tokens=allocation.context_window_size
        )

        # Create memory components
        working_memory = ContextMemory(
            name=f"{agent_name}_working_memory", max_size=allocation.working_memory_size
        )
        ...
        working_memory_tokens = int(
            allocation.context_window_size * allocation.working_memory_token_ratio
        )
        episodic_memory_tokens = int(
            allocation.context_window_size * allocation.episodic_memory_token_ratio
        )
        semantic_memory_tokens = int(
            allocation.context_window_size * allocation.semantic_memory_token_ratio
        )
        procedural_memory_tokens = int(
            allocation.context_window_size * allocation.procedural_memory_token_ratio
        )

        # Log configuration
        logger.info(
            f"Configured memory for agent '{agent_name}' with role '{role}' using model '{model_name}'"
        )
        logger.info(f"Context window size: {allocation.context_window_size} tokens")
        logger.info(
            f"Working memory: {allocation.working_memory_size} items, {working_memory_tokens} tokens"
        )
        logger.info(
            f"Episodic memory: {allocation.episodic_memory_size} episodes, {episodic_memory_tokens} tokens"
        )
        logger.info(
            f"Semantic memory: {allocation.semantic_memory_size} concepts, {semantic_memory_tokens} tokens"
        )
    ...
    def register_model(self, model_name: str, context_window_size: int) -> None:
        ...
        logger.info(
            f"Registered context window size {context_window_size} for model '{model_name}'"
        )
