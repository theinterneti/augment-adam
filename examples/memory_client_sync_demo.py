#!/usr/bin/env python3
"""
Synchronous Memory Client Demo

This script demonstrates how to use the synchronous memory client to interact with the memory service
...
def run_rest_demo(api_url: str, api_key: Optional[str] = None):
    ...
    """
    logger.info("=== Synchronous REST API Client Demo ===")
    
    # Create client
    client = SyncMemoryClient.create(ClientType.REST, api_url, api_key)
    ...
    try:
        ...
        result = client.add_memory(
            text="This is a memory from the synchronous REST API client",
            metadata={"source": "sync_rest_api_client", "importance": 0.9}
        )
        ...
        result = client.update_memory(
            memory_id=memory_id,
            text="This is an updated memory from the synchronous REST API client",
            metadata={"source": "sync_rest_api_client", "importance": 1.0}
        )
        logger.info(f"Updated memory: {json.dumps(result, indent=2)}")
        
        # List all memories
        logger.info("Listing all memories...")
        result = client.list_memories(limit=10)
        logger.info(f"Memories: {json.dumps(result, indent=2)}")
        
        # Search memories
        logger.info("Searching memories...")
        result = client.search_memories(query="synchronous REST API client")
...
def run_mcp_demo(mcp_url: str):
    """Run the MCP demo.
    
    Args:
        mcp_url: URL of the MCP server
    """
    logger.info("=== Synchronous MCP Client Demo ===")
    
    # Create client
    client = SyncMemoryClient.create(ClientType.MCP, mcp_url)
    ...
    try:
        ...
        result = client.add_memory(
            text="This is a memory from the synchronous MCP client",
            metadata={"source": "sync_mcp_client", "importance": 0.9}
        )
        logger.info(f"Added memory: {json.dumps(result, indent=2)}")
        ...
        result = client.update_memory(
            memory_id=memory_id,
            text="This is an updated memory from the synchronous MCP client",
            metadata={"source": "sync_mcp_client", "importance": 1.0}
        )
        logger.info(f"Updated memory: {json.dumps(result, indent=2)}")
        ...
        # Search memories
        logger.info("Searching memories...")
        result = client.search_memories(query="synchronous MCP client")
        logger.info(f"Search results: {json.dumps(result, indent=2)}")
...
def run_demo_with_context_manager(api_url: str, mcp_url: str):
    ...
    Args:
        ...
        mcp_url: URL of the MCP server
    """
    logger.info("=== Synchronous Context Manager Demo ===")
    
    # Using REST API client with context manager
    logger.info("Using REST API client with context manager...")
    with SyncMemoryClient.create(ClientType.REST, api_url) as client:
        result = client.add_memory(
            text="Memory created with synchronous context manager (REST)",
            metadata={"source": "sync_context_manager", "type": "rest"}
        )
        logger.info(f"Added memory: {json.dumps(result, indent=2)}")
    
    # Using MCP client with context manager
    logger.info("Using MCP client with context manager...")
    with SyncMemoryClient.create(ClientType.MCP, mcp_url) as client:
        result = client.add_memory(
            text="Memory created with synchronous context manager (MCP)",
            metadata={"source": "sync_context_manager", "type": "mcp"}
        )
        logger.info(f"Added memory: {json.dumps(result, indent=2)}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Synchronous Memory Client Demo")
    ...
    if args.client in ["rest", "both"]:
        run_rest_demo(args.api_url, api_key)
    
    if args.client in ["mcp", "both"]:
        run_mcp_demo(args.mcp_url)
    
    # Run the context manager demo
    run_demo_with_context_manager(args.api_url, args.mcp_url)
