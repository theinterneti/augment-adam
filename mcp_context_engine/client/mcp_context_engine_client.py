...
"""Client library for the MCP-enabled context engine."""
...
class MCPContextEngineClient:
    """Client for the MCP-enabled context engine."""
    ...
    def vector_search(self, query: str, k: int = 10, include_metadata: bool = True, 
        ...
        response = requests.post(f"{self.base_url}/api/vector/search", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
    ...
    def vector_store(self, text: str, metadata: Dict[str, Any], tier: str = "hot", 
        ...
        response = requests.post(f"{self.base_url}/api/vector/store", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
    ...
    def code_index(self, code: str, file_path: str, language: Optional[str] = None, 
        ...
        response = requests.post(f"{self.base_url}/api/code/index", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
    ...
    def create_relationship(self, from_id: str, to_id: str, relationship_type: str, 
        ...
        response = requests.post(f"{self.base_url}/api/graph/relationship", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
    ...
    def get_related_vectors(self, vector_id: str, relationship_type: Optional[str] = None, 
        ...
        response = requests.post(f"{self.base_url}/api/graph/related", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MCP Context Engine Client")
    parser.add_argument("--url", default="http://localhost:8080", help="API URL")
    parser.add_argument("--key", default="test-api-key", help="API key")
    parser.add_argument("command", choices=["health", "search", "store", "index", "repository", "mcp-tools"], help="Command to run")
    ...
    client = MCPContextEngineClient(args.url, args.key)
