...
from augment_adam.ai_agent.coordination.task import TaskResult, TaskStatus
...
class SimpleAggregator(ResultAggregator):
    ...
    a simple strategy, such as taking the first successful result or concatenating
    all results.
    ...
    def __init__(
        self, name: str = "simple_aggregator", strategy: str = "first_success"
    ) -> None:
        """
        ...
        Args:
            name: The name of the aggregator.
            strategy: The aggregation strategy to use ("first_success", "last_success",
                "concatenate").
        """
        super().__init__(name)
    ...
    def aggregate(self, results: List[TaskResult]) -> TaskResult:
        ...
        if self.strategy == "first_success":
            ...
            return TaskResult(
                ...
                metadata={
                    "aggregated_from": [
                        result.agent_id for result in successful_results
                    ]
                }
            )
...
class WeightedAggregator(ResultAggregator):
    ...
    def aggregate(self, results: List[TaskResult]) -> TaskResult:
        ...
        weighted_results = [
            (result, self.get_weight(result.agent_id)) for result in successful_results
        ]
        ...
        # Normalize weights
        normalized_weights = [
            (result, weight / total_weight) for result, weight in weighted_results
        ]

        # Combine outputs based on weights
        if all(
            isinstance(result.output, (int, float)) for result, _ in normalized_weights
        ):
            # For numeric outputs, calculate weighted average
            weighted_output = sum(
                result.output * weight for result, weight in normalized_weights
            )
            ...
            return TaskResult(
                ...
                metadata={
                    ...
                    "weights": {
                        result.agent_id: weight for result, weight in normalized_weights
                    }
                }
            )

        elif all(
            isinstance(result.output, str) for result, _ in normalized_weights
        ):
            # For string outputs, concatenate with weights as prefixes
            weighted_output = "\n\n".join(
                f"[Weight: {weight:.2f}] {result.output}"  # Format with weight
            ...
            isinstance(result.output, list) for result, _ in normalized_weights
        ):
            # For list outputs, create a weighted list of items
            weighted_output = []
            for result, weight in normalized_weights:
                for item in result.output:
                    # Add item with weight and source info
            ...
            # For mixed or unsupported output types, return the result with the
            # highest weight
...
class VotingAggregator(ResultAggregator):
    ...
    def __init__(
        self, name: str = "voting_aggregator", voting_method: str = "majority"
    ) -> None:
        """
    ...
    def aggregate(self, results: List[TaskResult]) -> TaskResult:
            ...
            for output_key, vote_data in votes.items():
                if vote_data["count"] > len(successful_results) / 2:
                    ...
                    return TaskResult(
                        ...
                        metadata={
                            "aggregated_from": [
                                r.agent_id for r in vote_data["results"]
                            ],
        ...
        if self.voting_method == "plurality":
            ...
            best_output_key = max(
                votes.keys(), key=lambda k: votes[k]["count"]
            )
