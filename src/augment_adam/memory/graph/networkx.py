...
class RelationshipJSONEncoder(json.JSONEncoder):
    """Custom JSON encoder that handles Relationship enum values."""
...
class NetworkXMemory(GraphMemory[GraphMemoryItem]):
    ...
    def save(self, directory: str) -> None:
        ...
        with open(os.path.join(directory, "items.json"), "w") as f:
            json.dump(items_data, f, cls=RelationshipJSONEncoder)
        ...
        for item_id, graph in self.graphs.items():
            ...
            with open(os.path.join(directory, f"{item_id}.json"), "w") as f:
                json.dump(graph_data, f, cls=RelationshipJSONEncoder)
