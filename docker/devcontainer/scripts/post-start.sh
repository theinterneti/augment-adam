...
if docker ps | grep -q "mcp-context-engine"; then
    ...
    if [ -f /workspace/docker/services/mcp-context-engine/scripts/setup_integration.sh ]; then
        echo "Setting up MCP Context Engine integration..."
        bash /workspace/docker/services/mcp-context-engine/scripts/setup_integration.sh || echo "Failed to set up MCP Context Engine integration"
    fi
else
    echo "MCP Context Engine services are not running."
    echo "You can start them with: /workspace/docker/scripts/manage_mcp.sh start"
fi
