...
def test_semantic_memory_add_concept(mock_client_class, mock_chroma_client, mock_chroma_collection):
    ...
    assert kwargs["documents"] == ["This is a test concept with detailed content"]
    ...
    with pytest.raises(ValueError):
        memory.add_concept(name="", description="A test concept", content="Content")
    ...
    with pytest.raises(ValueError):
        memory.add_concept(name="Test", description="A test concept", content="")
...
def test_semantic_memory_search_concepts(mock_client_class, mock_chroma_client, mock_chroma_collection):
    ...
    memory.search_concepts("test query", n_results=10, filter_metadata={"category": "test"})
...
def test_semantic_memory_get_all_concepts(mock_client_class, mock_chroma_client, mock_chroma_collection):
    ...
    assert concepts[0].name == "Concept A"  # First alphabetically
    assert concepts[0].description == "Description A"
    assert concepts[0].timestamp == timestamp2
    assert concepts[0].updated_at == updated_at2
    
    assert concepts[1].name == "Concept B"  # Second alphabetically
    assert concepts[1].description == "Description B"
    assert concepts[1].timestamp == timestamp1
    assert concepts[1].updated_at == updated_at1
    ...
    # Check that the concepts were retrieved correctly
    assert len(concepts) == 2
    assert concepts[0].name == "Concept B"  # Second alphabetically
    assert concepts[1].name == "Concept A"  # First alphabetically
...
def test_semantic_memory_get_recently_updated_concepts(mock_client_class, mock_chroma_client, mock_chroma_collection):
    ...
    memory.get_recently_updated_concepts(n=2, filter_metadata={"category": "test"})
    mock_chroma_collection.get.assert_called_with(
        where={"category": "test"},
        include=["documents", "metadatas", "ids"],
    )
...
def test_semantic_memory_get_related_concepts(mock_client_class, mock_chroma_client, mock_chroma_collection):
    ...
    mock_chroma_collection.query.return_value = {
        ...
        "distances": [[0.0, 0.5]],  # First one is the concept itself (distance 0)
    }
    ...
    assert kwargs["query_texts"] == ["This is a test concept with detailed content"]
    assert kwargs["n_results"] == 2  # n_results + 1 to account for the concept itself
